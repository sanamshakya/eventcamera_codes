...
// Main Cuda kernel for Generating events 
// One thread per pixel, no cross-pixel dependencies -> fully parallel.
// Algorithm : Calculates in two stage
// Polarity estimation : delta - change in intensity in pixel
// Positive and negative polarity based on delta
// Timestamp estimation : alpha - rate of change in intensity
                                      ...
                                      uint8_t *outputBuffer /* [int count][Event...] */)
{
    ...
    const float delta = currentIntensity - state.referenceIntensity; //change in intensity
