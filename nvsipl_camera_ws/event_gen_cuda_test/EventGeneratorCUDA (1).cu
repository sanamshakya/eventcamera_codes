...
// Output buffer layout is [int eventCount][Event events[...]]. `Event`
// contains a `double timestamp`, so alignof(Event) is 8 on essentially
// every real platform -- placing the Event array right after a 4-byte
// int (offset 4) misaligns every Event to 4-byte instead of 8-byte
// boundaries. CPUs silently tolerate that; GPUs trap on it, which is the
// "misaligned address" error surfacing at the next sync point. Round the
// header up to a multiple of alignof(Event) (computed, not hardcoded, so
// this self-adjusts if your real Event struct's layout differs) so the
// array that follows is always naturally aligned.
