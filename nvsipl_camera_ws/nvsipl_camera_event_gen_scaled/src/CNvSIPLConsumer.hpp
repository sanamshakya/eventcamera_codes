...
public:
    ...
    // srcTop must already point at the first row of the ACTIVE image
    // (i.e. raw16.data() + activeTopRow * srcStride), not the buffer start.
...
private:
    ...
    SIPLStatus CaptureRawBayerBuffer(INvSIPLClient::INvSIPLNvMBuffer *pNvMBuffer,
                                     ...
                                     int &outActiveTopRow, int &outActiveHeight)
    {
        NvSciError sciErr;
        SIPLStatus status;
        ...
        outActiveTopRow = static_cast<int>(bufAttrs.topPadding[0]);
    	outActiveHeight = static_cast<int>(height) - static_cast<int>(bufAttrs.topPadding[0])
                                               - static_cast<int>(bufAttrs.bottomPadding[0]);
