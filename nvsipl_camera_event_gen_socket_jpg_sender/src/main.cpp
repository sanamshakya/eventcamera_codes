...
std::atomic<bool> bLinkActive[MAX_NUM_SENSORS];
...
bool g_sc7_boot_enabled = false;
uint64_t g_sc7_resume_timestamp;
const char first_frame_test_results[] = "/tmp/nvsipl_camera_first_frame_test_results.txt";
...
static uint64_t get_timestamp_ms(void)
{
    ...
    timestamp_ms = sec_to_ms(tv.tv_sec);
    timestamp_ms += us_to_ms(tv.tv_usec);

    return timestamp_ms;
}
static int set_time_to_receive_first_frame(uint64_t ms)
{
    int rc;
    FILE* fptr;

    fptr = fopen(first_frame_test_results, "w");
...
static int reset_time_to_receive_first_frame(void) {
    int rc;

    rc = set_time_to_receive_first_frame(0);

    return rc;
}
static int record_time_to_receive_first_frame(void) {
    uint64_t first_frame_timestamp_ms;
    uint64_t time_to_receive_first_frame_after_sc7_resume_ms;
    int rc;

    first_frame_timestamp_ms = get_timestamp_ms();
    time_to_receive_first_frame_after_sc7_resume_ms = first_frame_timestamp_ms - g_sc7_resume_timestamp;

    rc = set_time_to_receive_first_frame(time_to_receive_first_frame_after_sc7_resume_ms);

    std::cout << "Time to receive first image frame after sc7 resume: " << time_to_receive_first_frame_after_sc7_resume_ms << "ms" << std::endl;

    return rc;
}
static int remove_time_to_receive_first_frame(void) {
    remove(first_frame_test_results);
...
private:
    ...
    void OnEvent(NotificationData &oNotificationData)
    {
        ...
        case NOTIF_ERROR_SENSOR_FAILURE:
            ...
            break;
        case NOTIF_ERROR_INTR_LOCALIZATION_FAILURE:
            LOG_ERR("DeviceBlock: %u, NOTIF_ERROR_INTR_LOCALIZATION_FAILURE\n", m_uDevBlkIndex);
            m_bInError = true;
            break;
        case NOTIF_ERROR_INTR_LOCALIZATION_TIMEOUT:
            LOG_ERR("DeviceBlock: %u, NOTIF_ERROR_INTR_LOCALIZATION_TIMEOUT\n", m_uDevBlkIndex);
            m_bInError = true;
            break;
        case NOTIF_ERROR_INTERNAL_FAILURE:
            LOG_ERR("DeviceBlock: %u, NOTIF_ERROR_INTERNAL_FAILURE\n", m_uDevBlkIndex);
            if (!bIgnoreError && (oNotificationData.uLinkMask != 0U)) {
                for (uint32_t i = 0; i < m_oDeviceBlockInfo.numCameraModules; i++) {
                    if (isTrueGPIOInterrupt(oNotificationData.gpioIdxs, oNotificationData.numGpioIdxs)) {
                        HandleCameraModuleError(m_oDeviceBlockInfo.cameraModuleInfoList[i].sensorInfo.id);
                    }
...
public:
    ...
    SIPLStatus Init(uint32_t uSensor,
                    INvSIPLNotificationQueue *notificationQueue,
                    DeviceBlockInfo &oDeviceBlockInfo,
    ...
    //! Get number of frame captured for this pipeline
    uint32_t GetNumFrameCaptured()
    {
        return m_uNumFrameCaptured;
    }
...
private:
    ...
    void OnEvent(NotificationData &oNotificationData)
    {
        ...
        case NOTIF_ERROR_ICP_AUTH_FAILURE:
            LOG_ERR("Pipeline: %u, ICP_AUTH_FAILURE frame=%lu\n",
                    oNotificationData.uIndex, oNotificationData.frameSeqNumber);
            break;
        case NOTIF_ERROR_ICP_AUTH_OUT_OF_ORDER:
    ...
    uint32_t m_uNumFrameCaptured = 0U;
...
public:
    ...
    static void FrameCompletionQueueThreadFunc(CPipelineFrameQueueHandler *pThis)
    {
        ...
        static bool firstFrameReceivedAfterSc7Resume = false;
        ...
        while ((!pThis->m_bQuit) || bFramesAvailable) {
            ...
            for (auto &queueConsPair : pThis->m_vQueueConsumerPair) {
                ...
                if (status == NVSIPL_STATUS_OK) {
                    ...
                    if ((g_sc7_boot_enabled) && (!firstFrameReceivedAfterSc7Resume)) {
                        record_time_to_receive_first_frame();
                        firstFrameReceivedAfterSc7Resume = true;
                    }
...
static void processOV2311CustomCommand(char *line) {
    ...
    OV2311NonFuSaCustomInterface* ov2311NonFuSaCustomInterface =
        upMaster->GetOV2311NonFuSaCustomInterface(0);

    if (ov2311NonFuSaCustomInterface == NULL) {
        return;
    }
...
static void processIMX728CustomCommand(char *line) {
    SIPLStatus status = NVSIPL_STATUS_OK;

    uint32_t param = 0;
    if ((line[0] == 'g') && (line[1] == 'c')) {
        uint8_t i = 2U; /* Starting point on the line */
        // Obtain set value from provided string
        while (line[i] != '\0' && i < MAX_LINE_LENGTH) {
            if ((line[i] >= '0') && (line[i] <= '9')) {
                param = (param * 10U) + (uint32_t)(line[i] - '0');
        ...
        // Get custom interface
        IMX728_CustomInterface* imx728_CustomInterface =
            upMaster->GetIMX728_CustomInterface(param);
        if (imx728_CustomInterface == nullptr) {
            LOG_ERR("Failed to obtain IMX728 custom interface\n");
            return;
        } else {
            LOG_DBG("IMX728 custom interface found\n");
            double current;
            status = imx728_CustomInterface->GetHeaterCurrent(current);
            if (status == NVSIPL_STATUS_OK) {
                LOG_MSG("Current Value: %.4f\n", current);
            }  else {
                LOG_ERR("Failed to get current value\n");
            }
        }
    } else if ((line[0] == 's') && (line[1] == 'h') && ((line[2] == '1') || (line[2] == '0'))) {
        uint8_t i = 3U; /* Starting point on the line */
        // Obtain set value from provided string
        while (line[i] != '\0' && i < MAX_LINE_LENGTH) {
            if ((line[i] >= '0') && (line[i] <= '9')) {
                param = (param * 10U) + (uint32_t)(line[i] - '0');
        ...
        if (imx728_CustomInterface == nullptr) {
            ...
            if (line[2] == '0') {
                status = imx728_CustomInterface->SetHeaterState(false);
                if (status == NVSIPL_STATUS_OK) {
                    LOG_MSG("Heater Off\n");
                }  else {
                    LOG_ERR("Failed to turn off heater\n");
                }
            } else if (line[2] == '1') {
                status = imx728_CustomInterface->SetHeaterState(true);
                if (status == NVSIPL_STATUS_OK) {
                    LOG_MSG("Heater On\n");
                }  else {
                    LOG_ERR("Failed to turn on heater\n");
                }
            } else {
              LOG_ERR("Invalid parameter for setting heater\n");
            }
        }
    } else {
         LOG_ERR("Invalid command for IMX728 custom interface\n");
    }
}
...
static void suspend_drive_os(void)
{
    uint64_t start_ms;
    uint64_t finish_ms;
    uint64_t time_in_suspend_ms;
    std::cout << "Suspending DriveOS..." << std::endl;
    start_ms = get_timestamp_ms();
#ifdef NVMEDIA_QNX
    nvdvms_set_vm_state(NVDVMS_SUSPEND);
#else
    system("echo 1 > /sys/class/tegra_hv_pm_ctl/tegra_hv_pm_ctl/device/trigger_sys_suspend");
    sleep(5);
#endif
    finish_ms = get_timestamp_ms();
    g_sc7_resume_timestamp = get_timestamp_ms();
    time_in_suspend_ms = finish_ms - start_ms;
...
int main(int argc, char *argv[])
{
    ...
    if (cmdline.bFsyncProgramStartTime) {
        LOG_INFO("Program and start fsync signal for group: %u with start_time %llu\n", cmdline.uFsyncGroupId, cmdline.uStartTimeTSCTicks);
        CAM_FSYNC_STATUS fsync_status = cam_fsync_program_abs_start_value(cmdline.uFsyncGroupId, cmdline.uStartTimeTSCTicks);
        if (fsync_status != CAM_FSYNC_OK) {
            LOG_ERR("Failed to program and start fsync signal for group: %u with status: %d\n", cmdline.uFsyncGroupId, fsync_status);
            return -1;
        }
    }
    ...
    if (!cmdline.vInputRawFiles.empty()) {
        ...
        oPlatformCfg.deviceBlockList[0].cameraModuleInfoList[0].isSimulatorModeEnabled = true;
    ...
    for (auto d = 0u; d != oPlatformCfg.numDeviceBlocks; d++) {
        ...
        /* Force-enable authentication for all modules if requested by a user */
        if (cmdline.bEnableAuth) {
            for (auto m = 0u; m < oPlatformCfg.deviceBlockList[d].numCameraModules; m++) {
                oPlatformCfg.deviceBlockList[d].cameraModuleInfoList[m].sensorInfo.isAuthEnabled = true;
            }
        }
    }
    ...
    if (cmdline.bTwoPassIsp) {
        ...
        oPlatformCfg.deviceBlockList[uSimulatorDevBlk].cameraModuleInfoList[0].isSimulatorModeEnabled = true;
    ...
    for (auto d = 0u; d != oPlatformCfg.numDeviceBlocks; d++) {
        if (cmdline.uCamRecCfg == 1U) {
            oPlatformCfg.deviceBlockList[d].deserInfo.name.append("_camRecCfg_V1");
        } else if (cmdline.uCamRecCfg == 2U) {
            oPlatformCfg.deviceBlockList[d].deserInfo.name.append("_camRecCfg_V2");
        }
    }
    ...
    for (auto d = 0u; d != oPlatformCfg.numDeviceBlocks; d++) {
        ...
        for (auto m = 0u; m != db.numCameraModules; m++) {
            ...
            pipelineCfg.disableSubframe = cmdline.bDisableSubframe;
            ...
            if (((eOutputList.size() + activeSensorCount) > 2U) &&
                (bDisplay)) {
                /* It is not possible to stream multiple outputs from sensors into Display
                ...
                downscaleCropCfg.isp0DownscaleWidth = 960U;
                downscaleCropCfg.isp0DownscaleHeight = 540U;
                downscaleCropCfg.isp1DownscaleEnable = true;
                downscaleCropCfg.isp1DownscaleWidth = 960U;
                downscaleCropCfg.isp1DownscaleHeight = 540U;

                if (!cmdline.bIsRGBIRsensor) {
                    downscaleCropCfg.isp2DownscaleEnable = true;
                    downscaleCropCfg.isp2DownscaleWidth = 960U;
                    downscaleCropCfg.isp2DownscaleHeight = 540U;
                }
            }

            if (cmdline.bIsRGBIRsensor) {
                downscaleCropCfg.isp1DownscaleEnable = true;
                downscaleCropCfg.isp1DownscaleWidth = sensor.vcInfo.resolution.width/2U;
                downscaleCropCfg.isp1DownscaleHeight = sensor.vcInfo.resolution.height/2U;
            }
            ...
            if (db.cameraModuleInfoList[m].isSimulatorModeEnabled && !cmdline.bTwoPassIsp) {
            ...
            if ((sensor.vcInfo.inputFormat == NVSIPL_CAP_INPUT_FORMAT_TYPE_RAW6) or
                ...
                (sensor.vcInfo.inputFormat == NVSIPL_CAP_INPUT_FORMAT_TYPE_RAW12RJ) or
            ...
            for (INvSIPLClient::ConsumerDesc::OutputType eOutput : eOutputList) {
                ...
                if (upComposite != nullptr) {
                    ...
                    status = upComposite->RegisterSource(grpIndex,
                                                         ...
                                                         isRawOutput,
                                                         uID,
                                                         db.cameraModuleInfoList[m].isSimulatorModeEnabled,
                ...
                // this is an empty variable that does not serve any purpose here, only kept
                // to avoid build issues with other apps.
                std::vector<uint64_t> uInitTimeArrUs;
                if (cmdline.bShowFPS || cmdline.bEnableProfiling || cmdline.bAutoRecovery) {
                    LOG_INFO("Profiler initialization for output:%u of sensor:%u\n", eOutput, uSensor);
                    bLinkActive[ uSensor ] = true;
                ...
                upCons->SetEventUdpStreaming(true, "192.168.1.10", 5005);//here 192.168.1.10 is the laptop IP running python visualization app
            ...
            status = upNotificationHandler->Init(uSensor, pipelineQueues.notificationQueue, db, vpProfilerISP);
    ...
    if (upComposite != nullptr) {
        ...
        status = upComposite->Init(cmdline.uNumDisplays,
                                   ...
                                   bufModule,
                                   syncModule,
                                   !((eOutputList.size() + activeSensorCount) > 2U));
        CHK_STATUS_AND_RETURN(status, "Compositor initialization");
    }
#endif // !NV_IS_SAFETY

    bool bTwoPassIspProducerAttrsRetrieved = false;
    for (auto d = 0u; d != oPlatformCfg.numDeviceBlocks; d++ ) {
        auto db = oPlatformCfg.deviceBlockList[d];
        for (auto m = 0u; m != db.numCameraModules; m++) {
            auto module = db.cameraModuleInfoList[m];
            ...
            uint32_t uSensor = sensor.id;
            {
                uint32_t uAllocPipeline = uSensor;
                if (cmdline.bTwoPassIsp) {
                    ...
                    if ((uSensor == uTwoPassIspConsumerId) && (!bTwoPassIspProducerAttrsRetrieved)) {
                        uAllocPipeline = uTwoPassIspProducerId;
                        bTwoPassIspProducerAttrsRetrieved = true;
                    } else if (uSensor == uTwoPassIspProducerId) {
                        if (bTwoPassIspProducerAttrsRetrieved) {
                            uAllocPipeline = uTwoPassIspConsumerId;
                        } else {
                            bTwoPassIspProducerAttrsRetrieved = true;
                        }
                    }
                }
                status = upMaster->AllocateBuffers(uAllocPipeline,
            ...
            if (!cmdline.bTwoPassIsp || (uSensor != uTwoPassIspConsumerId)) {
                status = upMaster->AllocateCPUWaiter(uSensor,
                                                        !cmdline.bDisableISP0,
                                                        !cmdline.bDisableISP1,
                                                        !cmdline.bDisableISP2);
                if (status != NVSIPL_STATUS_OK) {
                    LOG_ERR("AllocateCPUWaiter failed for sensor pipeline:%u\n", uSensor);
                    return -1;
                }
            }
        }
    ...
    if (cmdline.bEnableSc7Boot) {
        reset_time_to_receive_first_frame();
        g_sc7_boot_enabled = true;
        suspend_drive_os();
    } else {
        remove_time_to_receive_first_frame();
    }
    ...
    for (auto d = 0u; d != oPlatformCfg.numDeviceBlocks; d++) {
        ...
        for (auto m = 0u; m != db.numCameraModules; m++) {
            ...
            if (cmdline.bShowEEPROM) {
                LOG_MSG("Read EEPROM of Sensor %d\n", uSensor);
                upMaster->ShowEEPROM(uSensor);
            }
#if !NV_IS_SAFETY
            if (cmdline.bWriteEEPROM) {
                LOG_MSG("Write EEPROM of Sensor %d\n", uSensor);
                bool writeEEPROM = true;
                upMaster->ShowEEPROM(uSensor, writeEEPROM);
            }
            ...
            if (cmdline.bNvSci) {
                ...
                uint32_t uRegisterPipeline = uSensor;
                if (cmdline.bTwoPassIsp) {
                    // If necessary, for two-pass ISP processing swap the index used for
                    // allocation/registration so that the producer buffers get allocated and
                    // registered before the consumer buffers. This is necessary so that the
                    ...
                    if ((uSensor == uTwoPassIspConsumerId) && (!bTwoPassIspProducerRegistered)) {
                        uRegisterPipeline = uTwoPassIspProducerId;
                        bTwoPassIspProducerRegistered = true;
                    } else if (uSensor == uTwoPassIspProducerId) {
                        if (bTwoPassIspProducerRegistered) {
                            uRegisterPipeline = uTwoPassIspConsumerId;
                        } else {
                            bTwoPassIspProducerRegistered = true;
                        }
                    }
                }
                status = upMaster->RegisterBuffers(uRegisterPipeline,
                                                   oPlatformCfg.platformConfig,
                                                   !cmdline.bDisableRaw,
                                                   !cmdline.bDisableISP0,
                                                   !cmdline.bDisableISP1,
            ...
            if (!(cmdline.bNvSci && (upComposite != nullptr))) {
                 ...
                 * synchronization between SIPL and NvMedia 2D via Register2DSignalerWaiter().
                 * Otherwise, if there isn't a compositor, call RegisterCPUWaiter() to allocate
                 ...
                 * so in that case we always want to call RegisterCPUWaiter().
                 */
                if (upComposite != nullptr) {
                    status = upMaster->SetupNvSciSync2DSignalerWaiter(uSensor,
                                                                      ...
                                                                      db.cameraModuleInfoList[m].isSimulatorModeEnabled);
                    if (status != NVSIPL_STATUS_OK) {
                        LOG_ERR("Register2DSignalerWaiter failed for sensor pipeline:%u\n", uSensor);
                        return -1;
                    }
                } else {
#endif // !NV_IS_SAFETY
                    if (!cmdline.bTwoPassIsp || (uSensor != uTwoPassIspConsumerId)) {
                        status = upMaster->RegisterCPUWaiter(uSensor,
                                                             !cmdline.bDisableISP0,
                                                             !cmdline.bDisableISP1,
                                                             !cmdline.bDisableISP2);
                        if (status != NVSIPL_STATUS_OK) {
                            LOG_ERR("RegisterCPUWaiter failed for sensor pipeline:%u\n", uSensor);
                            return -1;
                        }
                    }
#if !NV_IS_SAFETY
    ...
    if ((upComposite != nullptr) && cmdline.bNvSci) {
        ...
        status = upMaster->SetupSync(bFileWrite || cmdline.bEnableProfiling);
        if (status != NVSIPL_STATUS_OK) {
    ...
    if (cmdline.bTwoPassIsp) {
        status = upMaster->SetTwoPassIspSync(uTwoPassIspProducerId,
        ...
        for (auto m = 0u; m != db.numCameraModules; m++) {
            ...
            if (!cmdline.bDisableISP0 ||
                ...
                if (cmdline.autoPlugin == NV_PLUGIN) {
                    ...
                    pluginlib_handle = dlopen("libnvsipl_sampleplugin.so", RTLD_NOW);
                    if (!pluginlib_handle) {
...
#endif
        for (auto d = 0u; d != oPlatformCfg.numDeviceBlocks; d++) {
           // Get custom interface for the deserializer
           // The client can get the custom interface handle
           // between SIPL Init() API and SIPL Start() API called
           MAX96712FusaNvCustomInterface* max96712FusaNvCustomInterface;
           uint32_t muxSel = 1U;
           uint32_t grpId = UINT32_MAX;
           uint32_t gpioNum[MAX_CAMERAMODULES_PER_BLOCK] = {2U, 2U, 2U, 2U};
           max96712FusaNvCustomInterface = upMaster->GetMAX96712FusaNVCustomInterface(d);
           if (max96712FusaNvCustomInterface != nullptr) {
               /* Find the camera group id */
               status = max96712FusaNvCustomInterface->GetGroupID(grpId);
               if (status != NVSIPL_STATUS_OK) {
                   LOG_ERR("Failed to get MAX96712 custom interface\n");
                   return -1;
               }
               /* Construct the multiplexer selection and GPIO index for the matched deserializer */
               if (grpId > 0U) {
                   muxSel = cmdline.uFsyncMuxDes[grpId - 1U];
               }
               for (uint8_t i = 0U; i < MAX_CAMERAMODULES_PER_BLOCK; i++) {
                   gpioNum[i] = cmdline.uGpioIdxDes[grpId][i];
               }

               LOG_DBG("MAX96712 Fusa NV custom interface found\n");
               status = max96712FusaNvCustomInterface->SetHeteroFrameSync(muxSel, gpioNum);
               if (status != NVSIPL_STATUS_OK) {
                   LOG_ERR("Failed to enable the Heterogeneous frame sync support\n");
                   return -1;
               }
           }
        }
    ...
    if (cmdline.uRunDurationSec >= 0) {
        LOG_WARN("Skipping runtime menu as runfor option is being used, runtime options are not going to be available\n");
    } else {
        ...
        std::thread([&]
        {
            ...
            while (!bQuit) {
                ...
                cout << "Enter 'cm' to check the module availability\n";
                ...
                if (cmdline.sConfigName.find("SIM728") != std::string::npos) {
                    // Show custom interface command
                    cout << "Enter 'sh<1 or 0>' followed by sensor ID to turn on or off heater\n";
                    cout << "Enter 'gc' followed by sensor ID to return current value in mA for heater\n";
                }
                ...
                if ((line[0] == 'c') && (line[1] == 'u') && (line[2] == 's') && (line[3] == 't')) {
                    //todo . add your own processing code block here.
                    processOV2311CustomCommand(line);
                }
                if(((line[0] == 'c') && (line[1] == 'm'))) {
                    uint32_t id = -1U;
                    id = atoi(&line[2]);
                    SIPLStatus status = upMaster->CheckSensorStatus(id);
                    if (status != NVSIPL_STATUS_OK) {
                        cout << "Sensor Status: " << id << "Error:"<<status << endl;
                    }
                    else {
                        cout << "Sensor Status: " << id << "OK" << endl;
                    }
                }
                if (((line[0] == 'g') && (line[1] == 'c')) ||
                    ((line[0] == 's') && (line[1] == 'h'))) {
                    processIMX728CustomCommand(line);
                }
            }
        }).detach();
    }
       ...
       // check if frames dumping is done
        ...
        if (cmdline.bShowFPS || cmdline.bEnableProfiling || cmdline.bAutoRecovery) {
            ...
            for (auto &prof : vupProfilers) {
                ...
                if (fps == 0 && bLinkActive[prof->m_uSensor]) {
                    if (m_upAutoRecovery != nullptr) {
                        m_upAutoRecovery->TriggerRecover(prof->m_uSensor);
                    }
                }
            }
    ...
    for (auto &notificationHandler : vupNotificationHandler) {
        cout << "Sensor" + to_string(notificationHandler->m_uSensor) + "\t\t"
             << "Frame captured: " << to_string(notificationHandler->GetNumFrameCaptured()) + "\t\t"
